...
interface AchievementsGlitchHeaderProps {
  className?: string;
}

export default function AchievementsGlitchHeader({ className = '' }: AchievementsGlitchHeaderProps) {
  ...
  useEffect(() => {
    ...
    const glitchInterval = setInterval(() => {
      if (Math.random() > 0.90) {
        ...
        setTimeout(() => setGlitchText(originalText), 140);
      }
    }, 170);
