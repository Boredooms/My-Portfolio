...
export default function SkillsHeader({ text, className = '' }: SkillsHeaderProps) {
  const originalText = 'SKILLS';
  const [displayText, setDisplayText] = useState(originalText);
  
  useEffect(() => {
    const interval = setInterval(() => {
      // More frequent glitching with 2-4 characters
      const numGlitch = Math.random() < 0.3 ? 2 : Math.random() < 0.6 ? 3 : 4;
    ...
    }, 900 + Math.random() * 1300); // Much more frequent: 0.9-2.2 seconds
