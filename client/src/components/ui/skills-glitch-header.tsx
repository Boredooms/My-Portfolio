...
interface SkillsGlitchHeaderProps {
  className?: string;
}

export default function SkillsGlitchHeader({ className = '' }: SkillsGlitchHeaderProps) {
  ...
  useEffect(() => {
    ...
    const glitchInterval = setInterval(() => {
      if (Math.random() > 0.91) {
        ...
        setTimeout(() => setGlitchText(originalText), 130);
      }
    }, 160);
